        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-4 gap-3">
            <div className="w-1/3 flex items-center gap-2">
               ...
               <div 
                 className={`hidden sm:flex items-center gap-2 px-3 py-1 rounded-full border text-xs cursor-help ${
                   !isOfflineMode 
                     ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' 
                     : 'bg-slate-700/50 border-slate-600 text-slate-400'
                 }`}
                 title={!isOfflineMode ? "Online" : "Local"}
               >
                 <div className={`w-1.5 h-1.5 rounded-full ${!isOfflineMode ? 'bg-emerald-500' : 'bg-slate-500'}`} />
                 <span>{!isOfflineMode ? 'Online' : 'Local'}</span>
               </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center gap-4">
            ...
            <div 
              className={`hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-mono cursor-help transition-colors ${
                !isOfflineMode 
                  ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' 
                  : 'bg-slate-700/50 border-slate-600 text-slate-400'
              }`}
              title={!isOfflineMode ? "Servidor Online: Seus dados estão sincronizados na nuvem." : "Modo Local: Seus dados estão salvos neste navegador."}
            >
              <div className={`w-2 h-2 rounded-full ${!isOfflineMode ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-slate-500'}`} />
              <span>{!isOfflineMode ? 'Online' : 'Local'}</span>
            </div>
                ...
                <>
                  ...
                  <div className="bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-white p-3 rounded-lg mt-2 flex justify-between items-center border border-slate-200 dark:border-slate-700">
                     <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs text-slate-500 dark:text-slate-400">{!isOfflineMode ? 'Online' : 'Offline'}</span>
                     </div>
                     <div className="flex items-center gap-1">
