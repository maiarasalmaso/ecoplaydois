...
const EDU_TIPS = [
  'Separar recicláveis reduz lixo em aterros e economiza energia.',
  'Energia solar é renovável e ajuda a reduzir emissões de CO₂.',
  'Levar sua garrafinha reutilizável evita centenas de plásticos por ano.',
  'Preferir alimentos locais reduz transporte e impacto ambiental.',
  'Compostagem transforma resíduos orgânicos em adubo para plantas.',
  'Economizar água também economiza energia usada no tratamento e bombeamento.',
];
...
export const MobileJoystick = ({ onDirection, disabled = false }) => {
  ...
  return (
    <div className="flex items-center justify-center select-none">
      <div
        ref={baseRef}
        className={`relative w-24 h-24 rounded-full border border-slate-700 bg-slate-900/50 backdrop-blur touch-none ${
...
const makeRandomObstacle = (gridSize) => {
  const pool = [
    { type: 'poluicao', color: '#fb7185' },
    { type: 'desmatamento', color: '#a16207' },
    { type: 'plastico', color: '#60a5fa' },
  ];
...
const spawnItem = (state) => {
  ...
  const wantsSpecial =
    state.foodsEaten > 0 && state.foodsEaten % 6 === 0 ? true : Math.random() < 0.1;
  const type = wantsSpecial ? pickOne(['solar', 'bin']) : pickOne(['organic', 'local']);
...
const applyCollect = (state, collectedType, nowMs) => {
  const next = { ...state };
  const streakContinues = nowMs - next.lastCollectAt <= 2500;
  next.streak = streakContinues ? next.streak + 1 : 1;
  next.lastCollectAt = nowMs;

  const ecoMultiplier = computeEcoMultiplier(next, nowMs);
  const addPoints = (base) => {
    next.score += Math.round(base * ecoMultiplier);
  };

  if (collectedType === 'organic') {
    addPoints(10);
    next.foodsEaten += 1;
  } else if (collectedType === 'local') {
    addPoints(15);
    next.foodsEaten += 1;
  } else if (collectedType === 'solar') {
    next.solarBoostUntil = Math.max(next.solarBoostUntil, nowMs + 6000);
    addPoints(30);
    next.foodsEaten += 1;
  } else if (collectedType === 'bin') {
    addPoints(20);
    return spawnRecyclablesChallenge(next, nowMs);
  } else if (collectedType === 'recyclable') {
    addPoints(8);
    next.challenge.remaining = Math.max(0, next.challenge.remaining - 1);
    if (next.challenge.remaining === 0) {
      addPoints(60);
      if (next.obstacles.length > 0) {
        const index = randomInt(0, next.obstacles.length);
        next.obstacles = next.obstacles.filter((_, i) => i !== index);
      }
  ...
  const nextLevel = 1 + Math.floor(next.foodsEaten / 4);
...
const tick = (state, nowMs) => {
  ...
  if (eatingItem) {
    const collectedType = state.item.type;
    nextState.item = null;
    nextState = applyCollect(nextState, collectedType, nowMs);
  } else if (eatingRecyclable) {
    nextState.recyclables = nextState.recyclables.filter((_, i) => i !== recyclableIndex);
    nextState = applyCollect(nextState, 'recyclable', nowMs);
  }

  return nextState;
};

const draw = (ctx, state, render, nowMs) => {
  ...
  for (const obstacle of state.obstacles) {
    const x = obstacle.x * cellPx;
    const y = obstacle.y * cellPx;
  ...
  if (state.item) {
    if (state.item.type === 'organic') drawToken(state.item.x, state.item.y, '#22c55e', '🍎');
    if (state.item.type === 'local') drawToken(state.item.x, state.item.y, '#84cc16', '🥕');
    if (state.item.type === 'solar') drawToken(state.item.x, state.item.y, '#f59e0b', '☀️');
    if (state.item.type === 'bin') drawToken(state.item.x, state.item.y, '#10b981', '♻️');
  }
...
const EcoSnake = () => {
  ...
  const [tip, setTip] = useState(() => pickOne(EDU_TIPS));
  ...
  useEffect(() => {
    const loop = (frameMs) => {
      ...
      draw(ctx, stateRef.current, render, nowMs);
    ...
    rafRef.current = requestAnimationFrame(loop);
    return () => cancelAnimationFrame(rafRef.current);
  }, [bestScore, ensureState, hydrateHud]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 text-white px-4 py-6 md:py-10">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/50 border border-slate-700">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/50 border border-slate-700 text-slate-200 hover:bg-slate-700/60 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
            ...
            <button
              type="button"
              onClick={resetGame}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/50 border border-slate-700 text-slate-200 hover:bg-slate-700/60 transition-colors"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6 items-start">
          <div className="relative">
            <div
              ...
              className="w-full max-w-[560px] mx-auto aspect-square rounded-2xl border border-slate-700 bg-slate-900/30 shadow-2xl overflow-hidden"
            >
            ...
            <AnimatePresence>
              {uiState !== 'playing' && (
                ...
                >
                  <div className="w-full max-w-md bg-slate-900/85 backdrop-blur-xl border border-slate-700 rounded-3xl p-6 shadow-2xl">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-10 h-10 rounded-2xl bg-eco-green/15 text-eco-green flex items-center justify-center">
                        <Leaf className="w-5 h-5" />
                      </div>
                      <div>
                        <h1 className="text-2xl font-display font-bold">{overlayTitle}</h1>
                        <div className="text-sm text-slate-400">
                          Pontos <span className="text-white font-mono">{hud.score}</span>
                          <span className="text-slate-600 mx-2">•</span>
                          Recorde <span className="text-white font-mono">{bestScore}</span>
                        </div>
                      </div>
                    </div>

                    <div className="text-slate-200 leading-relaxed mb-5">
                      <div className="text-slate-300">{tip}</div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <Link
                        to="/games"
                        className="inline-flex items-center justify-center gap-2 px-5 py-3 rounded-2xl bg-slate-800 text-white font-bold border border-slate-700 hover:bg-slate-700 transition-colors"
                      >
                        Voltar ao Arcade
                      </Link>
                    </div>
                    ...
                    <div className="mt-5 grid grid-cols-2 gap-3 text-xs text-slate-300">
                      <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-3">
                        <div className="text-slate-400 mb-1">Controles</div>
                        <div className="font-mono">↑ ↓ ← → / WASD</div>
                      </div>
                      <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-3">
                        <div className="text-slate-400 mb-1">Mobile</div>
          ...
          <div className="bg-slate-900/40 border border-slate-700 rounded-3xl p-5 space-y-4">
            <div className="flex items-center justify-between">
              <div className="text-slate-400 text-sm">Pontuação</div>
              <div className="text-2xl font-mono font-bold">{hud.score}</div>
            </div>

            <div className="grid grid-cols-3 gap-3">
              <div className="bg-slate-800/40 border border-slate-700 rounded-2xl p-3">
                <div className="text-slate-400 text-xs mb-1">Nível</div>
                <div className="font-mono text-lg">{hud.level}</div>
              </div>
              <div className="bg-slate-800/40 border border-slate-700 rounded-2xl p-3">
                <div className="text-slate-400 text-xs mb-1">Tamanho</div>
                <div className="font-mono text-lg">{hud.length}</div>
              </div>
              <div className="bg-slate-800/40 border border-slate-700 rounded-2xl p-3">
                <div className="text-slate-400 text-xs mb-1">Veloc.</div>
                <div className="font-mono text-lg">{Math.round(1000 / hud.stepMs)}/s</div>
              </div>
            </div>

            <div className="bg-slate-800/40 border border-slate-700 rounded-2xl p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="text-slate-300 font-bold">Bônus sustentável</div>
                <div className="font-mono text-slate-200">{formatMultiplier(hud.multiplier)}</div>
              </div>
              <div className="flex items-center gap-3 text-sm text-slate-300">
                <Sun className={`w-4 h-4 ${hud.solarMs > 0 ? 'text-amber-300' : 'text-slate-500'}`} />
              ...
              <div className="mt-3 flex items-center gap-3 text-sm text-slate-300">
                <Leaf className={`w-4 h-4 ${hud.multiplier > 1 ? 'text-eco-green' : 'text-slate-500'}`} />
                <div className="flex-1">
                  <div className="flex items-center justify-between">
            ...
            <div className="bg-slate-800/40 border border-slate-700 rounded-2xl p-4">
              <div className="text-slate-300 font-bold mb-2">Objetivos</div>
            ...
            <div className="rounded-2xl border border-slate-700 bg-slate-900/30 p-4 text-sm text-slate-300">
            ...
            <div className="grid grid-cols-3 gap-3 pt-2">
              <button
                ...
                className="col-start-2 px-3 py-3 rounded-2xl bg-slate-800/60 border border-slate-700 hover:bg-slate-700/70 transition-colors"
              >
                ↑
              </button>
              <button
                ...
                className="px-3 py-3 rounded-2xl bg-slate-800/60 border border-slate-700 hover:bg-slate-700/70 transition-colors"
              >
                ←
              </button>
              <button
