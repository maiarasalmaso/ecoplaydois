...
import { ArrowLeft, RefreshCw, AlertCircle, Trophy, Timer, Star, Home, Volume2, VolumeX, Lightbulb, CheckCircle2, HelpCircle, Settings } from 'lucide-react';
...
const EcoSudoku = () => {
  ...
  return (
    <div className={`min-h-screen ${theme.bg} ${theme.text} font-sans pb-20 pt-8 transition-colors duration-300`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        {/* Header Section */}
        <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4">
          <Link to="/games" className={`flex items-center gap-2 transition-colors self-start md:self-auto ${theme.subText} hover:${theme.heading}`}>
            <ArrowLeft className="w-5 h-5" />
            <span className="font-bold">Voltar</span>
          </Link>

          <div className="flex flex-col items-center">
            <h1 className={`text-3xl font-display font-bold ${theme.heading}`}>EcoSudoku</h1>
            <p className={theme.subText}>Complete com energias renováveis</p>
          </div>

          <div className={`flex items-center gap-4 ${theme.cardBg} p-2 rounded-xl shadow-sm border ${theme.cardBorder}`}>
            <div className={`flex items-center gap-2 px-3 py-1 rounded-lg ${isDarkMode ? 'bg-slate-800/50' : 'bg-slate-100'}`}>
              <Timer className="w-4 h-4 text-indigo-500" />
              <span className={`font-mono font-bold ${theme.text}`}>{formatTime(timeElapsed)}</span>
            </div>
            <div className={`flex items-center gap-2 px-3 py-1 rounded-lg ${isDarkMode ? 'bg-slate-800/50' : 'bg-slate-100'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-3 space-y-6 order-2 lg:order-1">
            ...
            <div className={`${theme.cardBg} p-6 rounded-2xl shadow-sm border ${theme.cardBorder}`}>
              ...
              <div className="space-y-4">
                ...
                {/* Theme Toggle Removed - Controlled by Header */}
          ...
          <div className="lg:col-span-6 flex flex-col items-center order-1 lg:order-2">
            <div className={`w-full max-w-[500px] aspect-square p-2 rounded-2xl shadow-xl transition-colors ${theme.boardOuter}`}>
              <div className={`w-full h-full grid grid-cols-9 gap-[1px] border-4 rounded-xl overflow-hidden ${theme.gridLines} ${theme.gridBorder}`}>
                {board.map((row, rowIndex) => (
                  row.map((cellValue, colIndex) => {
                    ...
                    const borderRight = (colIndex + 1) % 3 === 0 && colIndex !== 8 ? `border-r-2 ${theme.gridBorder}` : '';
                    const borderBottom = (rowIndex + 1) % 3 === 0 && rowIndex !== 8 ? `border-b-2 ${theme.gridBorder}` : '';
          ...
          <div className="lg:col-span-3 order-3">
            <div className={`${theme.cardBg} p-6 rounded-2xl shadow-sm border ${theme.cardBorder} sticky top-24`}>
